...
const AddAssignedForm = ({ assignation, onClose, rowData }) => {
  ...
  useEffect(() => {
    if (!effectMounted.current) {
      ...
      if (rowData) {
        ...
        if (rowData.status) {
          const selectedOption = options.find(option => option.id === rowData.status);
          if (selectedOption) {
            setSelectedStatus(selectedOption);
          } else {
            setSelectedStatus({ id: rowData.status, label: 'Estado desconocido' });
          }
        }
        if (rowData.chars) {
  ...
  const options = [
    { id: 1, label: 'En resguardo' },
    { id: 2, label: 'Asignado' },
    { id: 3, label: 'Obsoleto/Dañado' },
  ];

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-[#2C1C47] z-50 bg-opacity-30 px-5">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full md:w-[500px] relative">
        ...
        <div className=' mt-5'>
          ...
          <div className=' mt-5'>
          <Listbox value={selectedStatus} onChange={setSelectedStatus} className="max-w-[100px] mb-4 text-black mt-5">
            {({ open }) => (
              <>
